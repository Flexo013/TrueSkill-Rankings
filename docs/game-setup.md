...
Everything on this page happens under the host account (see [Account setup](account-setup.md)). Follow these steps for each new game — including a second league of an existing game for a different group of colleagues, which is set up exactly the same way with its own spreadsheet, forms, and label.
