...
- Outside working hours the process exits, so it needs to be started each working day via cron on the host system. The entry currently in use (`-u` keeps the log unbuffered):

  ```
  55 8 * * 1-5  cd /home/floris.huizinga/Repos/Foosball && /usr/bin/python3 -u mail_checker.py >> /home/floris.huizinga/Repos/Foosball/logs/mail-checker-$(date +\%m-\%d).log 2>&1
  ```
