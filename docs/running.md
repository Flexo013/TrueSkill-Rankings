...
- Outside working hours the process exits, so it needs to be started each working day — for example via cron on the host system:

  ```
  0 8 * * 1-5  cd /path/to/repo && python3 mail_checker.py >> logs/mail-checker-$(date +\%m-\%d).log 2>&1
  ```
